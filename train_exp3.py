...
def train():
    ...
    lr = 2e-4  # High LR experiment (2x)
    save_every = 20   # 저장 주기 조정
    noise_level = 25
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"[EXP3 - High LR 2e-4] Device: {device}\n")
    ...
    # 학습
    os.makedirs('./checkpoints/exp3', exist_ok=True)
    losses, psnrs = [], []
    ...
    for epoch in range(num_epochs):
        ...
        if val_psnr > best_psnr:
            best_psnr = val_psnr
            best_epoch = epoch + 1
            torch.save(model.state_dict(), './checkpoints/exp3/model_best.pth')
            print(f"✓ Best 모델 저장! (Epoch {best_epoch}, PSNR: {best_psnr:.2f} dB)\n")
        
        # 주기적 모델 저장 및 그래프 업데이트
        if (epoch + 1) % save_every == 0:
            save_path = f'./checkpoints/exp3/model_epoch_{epoch+1}.pth'
            ...
            plt.tight_layout()
            plt.savefig('./checkpoints/exp3/training_curve.png', dpi=150)
            plt.close()
            print(f"✓ 그래프 업데이트: ./checkpoints/exp3/training_curve.png\n")
    
    # 최종 저장
    torch.save(model.state_dict(), './checkpoints/exp3/model_final.pth')
    print(f"\n{'='*50}")
