...
def train():
    ...
    print(f"[EXP2 - Long 1000 epochs] Device: {device}\n")
    ...
    # 학습
    os.makedirs('./checkpoints/exp2', exist_ok=True)
    losses, psnrs = [], []
    ...
    for epoch in range(num_epochs):
        ...
        if val_psnr > best_psnr:
            best_psnr = val_psnr
            best_epoch = epoch + 1
            torch.save(model.state_dict(), './checkpoints/exp2/model_best.pth')
            print(f"✓ Best 모델 저장! (Epoch {best_epoch}, PSNR: {best_psnr:.2f} dB)\n")
        
        # 주기적 모델 저장 및 그래프 업데이트
        if (epoch + 1) % save_every == 0:
            save_path = f'./checkpoints/exp2/model_epoch_{epoch+1}.pth'
            ...
            plt.tight_layout()
            plt.savefig('./checkpoints/exp2/training_curve.png', dpi=150)
            plt.close()
            print(f"✓ 그래프 업데이트: ./checkpoints/exp2/training_curve.png\n")
    
    # 최종 저장
    torch.save(model.state_dict(), './checkpoints/exp2/model_final.pth')
    print(f"\n{'='*50}")
