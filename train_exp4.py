...
def train():
    ...
    batch_size = 128  # Big batch experiment (2x)
    lr = 1e-4  # KAIR 설정
    save_every = 20   # 저장 주기 조정
    noise_level = 25
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"[EXP4 - Big Batch 128] Device: {device}\n")
    ...
    for epoch in range(num_epochs):
        ...
        if val_psnr > best_psnr:
            best_psnr = val_psnr
            best_epoch = epoch + 1
            torch.save(model.state_dict(), './checkpoints/exp4/model_best.pth')
            print(f"✓ Best 모델 저장! (Epoch {best_epoch}, PSNR: {best_psnr:.2f} dB)\n")
        
        # 주기적 모델 저장 및 그래프 업데이트
        if (epoch + 1) % save_every == 0:
            save_path = f'./checkpoints/exp4/model_epoch_{epoch+1}.pth'
            ...
            plt.tight_layout()
            plt.savefig('./checkpoints/exp4/training_curve.png', dpi=150)
            plt.close()
            print(f"✓ 그래프 업데이트: ./checkpoints/exp4/training_curve.png\n")
    
    # 최종 저장
    torch.save(model.state_dict(), './checkpoints/exp4/model_final.pth')
    print(f"\n{'='*50}")
